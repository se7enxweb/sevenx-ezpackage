...
{*          <p class="ex-vat">(price ex. vat {$node.object.data_map.price.content.ex_vat_price})</p> *}
	...
	{if $node.data_map.product_inventory_count.content|le(0)}<div class="warning-out-of-stock">Out of stock!</div>{/if}
            <input {if $node.data_map.product_inventory_count.content|le(0)}disabled{/if} type="submit" class="defaultbutton" name="ActionAddToBasket" value="{"Add to basket"|i18n("design/ezwebin/full/product")}" />
